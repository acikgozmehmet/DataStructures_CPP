...
{
    // It returns the first element which is equal to findMe in the array even though there may be some other same number in the array
    //  since the elements in the array are generated by random(). Is it logically right?
    // If findMe is not in the array, -1 will be the return value and search is carried out from the beginning to the end of the array
...
int Searcher::Search2( const vector<int>& arr, int findMe )
{
    ...
    for (int i = 1; i <= group; i++){
        ...
        if ( (findMe >= arr[start]) && (findMe <= arr[finish]) )
            break;
    }
